...
export function ProdutoPage() {
  ...
  const fetchPage = React.useCallback(async (nextQ: string, nextPage: number) => {
    ...
    try {
      ...
      else setError('Não foi possível carregar os produtos. A API está rodando?')
    } finally {
      setLoading(false)
    }
  }, [])
    ...
    <div className="flex flex-col gap-4 p-4 md:gap-6 md:p-6">
      <header className="flex flex-col gap-1">
        ...
        <p className="text-sm text-neutral-500">
          Itens do catálogo (matéria-prima, insumo, produto acabado, etc.).
        </p>
      </header>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <ExportXlsxButton
            ...
            rows={items}
            columns={[
              { header: 'Código', value: (p) => p.codigo, width: 16 },
      ...
      <Card className="overflow-hidden rounded-xl border-neutral-200 bg-white p-0 shadow-sm">
        <BulkDeleteToolbar
          ...
          entityLabel="produto"
          entityPlural="produtos"
          deleteOne={(id) => produtosApi.remove(id)}
          onComplete={() => {
            selection.clear()
            refresh()
          }}
              ...
              {!loading &&
                items.map((p) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 font-medium text-neutral-900">{p.codigo}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {editing
                ? 'Atualize o cadastro do produto.'
