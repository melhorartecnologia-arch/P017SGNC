...
import { TipoRelatorioForm } from './TipoRelatorioForm'
...
export function TipoRelatorioPage() {
  ...
  const [editing, setEditing] = React.useState<TipoRelatorio | null>(null)
  ...
  const fetchPage = React.useCallback(async (nextQ: string, nextPage: number) => {
    ...
    try {
      ...
      else setError('Não foi possível carregar os tipos de relatório. A API está rodando?')
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const openEdit = (t: TipoRelatorio) => {
    setEditing(t)
    setDialogOpen(true)
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeletingId(id)
    try {
      await tiposRelatorioApi.remove(id)
    ...
    <div className="flex flex-col gap-4 p-4 md:gap-6 md:p-6">
      <header className="flex flex-col gap-1">
        <div className="flex items-center gap-2">
          <Files className="h-5 w-5 text-neutral-500" />
          <h1 className="text-xl font-semibold tracking-tight text-neutral-900">
            Cadastro de Tipos de Relatórios
          </h1>
        </div>
        <p className="text-sm text-neutral-500">
          Categorias dos relatórios emitidos pelo sistema (ex.: RNC, RAQ, RHE, RVT).
        </p>
      </header>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          <ImportXlsxButton
            ...
            entityPlural="tipos de relatório"
            expectedHeaders={[
            ...
            importOne={(input) => tiposRelatorioApi.create(input)}
            onDone={refresh}
          />
          <ExportXlsxButton
            filename="tipos-relatorio"
            sheetName="Tipos de Relatórios"
            rows={items}
            columns={[
            ...
            total={total}
            fetchAll={() => fetchAllPaged((p) => tiposRelatorioApi.list({ q: q.trim() || undefined, ...p }))}
          />
      ...
      <Card className="overflow-hidden rounded-xl border-neutral-200 bg-white p-0 shadow-sm">
        <BulkDeleteToolbar
          ...
          entityLabel="tipo de relatório"
          entityPlural="tipos de relatório"
          deleteOne={(id) => tiposRelatorioApi.remove(id)}
          onComplete={() => {
            selection.clear()
            refresh()
          }}
              ...
              {!loading && items.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-3 py-10 text-center text-neutral-500">
                    Nenhum tipo de relatório cadastrado.
                  </td>
                </tr>
              )}
              {!loading &&
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editing ? 'Editar tipo de relatório' : 'Novo tipo de relatório'}
            </DialogTitle>
            <DialogDescription>
              {editing
                ? 'Atualize o cadastro do tipo de relatório.'
                : 'Cadastre um novo tipo de relatório.'}
            </DialogDescription>
          </DialogHeader>
          <TipoRelatorioForm
            initial={editing}
      ...
      <Dialog open={confirmingId !== null} onOpenChange={(o) => !o && setConfirmingId(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Excluir tipo de relatório?</DialogTitle>
            <DialogDescription>
