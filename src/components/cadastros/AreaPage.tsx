...
export function AreaPage() {
  const [items, setItems] = React.useState<Area[]>([])
  ...
  const fetchPage = React.useCallback(async (nextQ: string, nextPage: number) => {
    ...
    try {
      ...
      else setError('Não foi possível carregar as áreas. A API está rodando?')
    } finally {
      setLoading(false)
    }
  }, [])
    ...
    <div className="flex flex-col gap-4 p-4 md:gap-6 md:p-6">
      <header className="flex flex-col gap-1">
        ...
        <p className="text-sm text-neutral-500">
          Áreas organizacionais (Produção, Qualidade, etc.). Usadas pelos cadastros
          de aprovadores em cada filial.
        </p>
      </header>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          <ImportXlsxButton
            ...
            importOne={(input) => areasApi.create(input)}
            onDone={refresh}
          />
          <ExportXlsxButton
            filename="areas"
            ...
            rows={items}
            columns={[
              { header: 'Código', value: (a) => a.codigo, width: 14 },
              { header: 'Nome', value: (a) => a.nome, width: 28 },
              { header: 'Descrição', value: (a) => a.descricao ?? '', width: 48 },
              { header: 'Situação', value: (a) => (a.ativo ? 'Ativa' : 'Inativa'), width: 10 },
            ]}
            total={total}
            fetchAll={() => fetchAllPaged((p) => areasApi.list({ q: q.trim() || undefined, ...p }))}
          />
      ...
      <Card className="overflow-hidden rounded-xl border-neutral-200 bg-white p-0 shadow-sm">
        <BulkDeleteToolbar
          ...
          entityLabel="área"
          entityPlural="áreas"
          deleteOne={(id) => areasApi.remove(id)}
          onComplete={() => {
            selection.clear()
            refresh()
          }}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {editing
                ? 'Atualize o cadastro da área.'
                : 'Cadastre uma nova área organizacional.'}
            </DialogDescription>
          </DialogHeader>
          <AreaForm
            initial={editing}
      ...
      <Dialog open={confirmingId !== null} onOpenChange={(o) => !o && setConfirmingId(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              A área só pode ser excluída se não houver aprovadores vinculados a ela.
            </DialogDescription>
          </DialogHeader>
