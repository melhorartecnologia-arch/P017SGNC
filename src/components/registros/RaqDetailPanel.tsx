...
import { raqApi, pendenciasParaAssinaturaRaq, type Raq } from '@/lib/api/raq'
import { RncFotosSection } from './RncFotosSection'

/**
 * Painel de detalhes do RAQ. As ações de workflow (assinatura, lembrete,
 * escalonamento, PDF) usam os endpoints compartilhados de /api/rnc, que
 * servem os dois tipos de documento; após cada ação o painel recarrega o
 * RAQ para manter os campos próprios (título, relacionados, envio ao
 * fornecedor).
 */
...
export function RaqDetailPanel({ raq, onClose, onEdit, onUpdated }: Props) {
  const open = !!raq
  const [enviando, setEnviando] = React.useState(false)
  const [lembrando, setLembrando] = React.useState(false)
  const [escalonando, setEscalonando] = React.useState(false)
  const [baixando, setBaixando] = React.useState(false)
  const pendencias = raq ? pendenciasParaAssinaturaRaq(raq) : []
  const jaEnviado = !!raq && raq.status !== 'DRAFT'
  const temPendentes = !!raq && raq.aprovadores.some((a) => !a.assinadoEm)

  /** Recarrega o RAQ após uma ação de workflow (que devolve o tipo Rnc). */
  const recarregar = async () => {
    if (!raq) return
    try {
      onUpdated?.(await raqApi.get(raq.id))
    } catch {
  ...
  const handleEnviarAssinatura = async () => {
    ...
    setEnviando(true)
    try {
      const { enviados, falhas } = await rncApi.enviarParaAssinatura(raq.id)
      await recarregar()
      if (falhas.length === 0) {
        toast.success('RAQ enviado para assinatura', {
          description: `${enviados.length} e-mail(s) enviado(s) aos aprovadores do tipo RAQ.`,
        })
      } else {
  ...
  const handleLembrete = async () => {
    ...
    setLembrando(true)
    try {
      const { enviados } = await rncApi.enviarLembrete(raq.id)
      await recarregar()
  ...
  const handleEscalonar = async () => {
    ...
    setEscalonando(true)
    try {
      const { novos } = await rncApi.escalonar(raq.id)
      await recarregar()
  ...
  const handlePdf = async () => {
    ...
    setBaixando(true)
    try {
      await rncApi.downloadPdf(raq.id, raq.numero)
    } catch (err) {
      const message =
        ...
        {raq && (
          <>
            <div className="flex items-start justify-between gap-3 border-b border-neutral-100 px-6 py-4">
              <div className="flex min-w-0 flex-col gap-1">
                <span className="text-[11px] font-semibold uppercase tracking-wider text-neutral-400">
                  Relatório de Alerta de Qualidade
                </span>
                <h2 className="truncate text-2xl font-semibold tracking-tight text-neutral-900">
                  RAQ <span className="tabular-nums">{raq.numero}</span>
                </h2>
                {raq.titulo && (
                  <p className="truncate text-sm text-neutral-600">{raq.titulo}</p>
                )}
                <span className="inline-flex items-center gap-1.5 text-xs font-medium text-neutral-600">
                  <span
                    className={cn(
                      'inline-block h-1.5 w-1.5 rounded-full',
                      STATUS_DOT[raq.status],
                    )}
                  />
                  {STATUS_LABELS[raq.status]}
                </span>
              </div>
                ...
                <Button
                  ...
                  title={
                    pendencias.length > 0
                      ...
                      : 'Enviar e-mail de assinatura aos aprovadores do tipo RAQ'
                  }
                >
                  {enviando ? (
            ...
            <div className="flex-1 overflow-y-auto px-6">
              {/* Conclusão: envio do documento assinado ao fornecedor */}
              <Section title="Envio ao fornecedor">
                {raq.enviadoFornecedorEm ? (
                  <div className="flex items-start gap-2 rounded-md border border-emerald-200 bg-emerald-50 px-2.5 py-2 text-xs text-emerald-800">
                    ...
                    <span>
                      ...
                      <b>{raq.enviadoFornecedorPara}</b> em{' '}
                      {fmtDataHora(raq.enviadoFornecedorEm)}. O fluxo do RAQ se
                      encerra com este envio — não há resposta do fornecedor.
                    </span>
                  </div>
                ) : (
              ...
              <Section
                title={`Matriz de aprovação · ${resumoAssinaturas(raq).label}`}
              >
                {raq.aprovadores.length === 0 ? (
                  <p className="text-[13px] text-neutral-500">
                    Nenhum aprovador configurado para o tipo RAQ nesta filial.
                    Vincule o tipo RAQ aos aprovadores no cadastro de
                    Aprovadores.
                  </p>
                ) : (
                  raq.aprovadores.map((a) => (
                    <div
                      key={a.id}
              ...
              <Section title="Identificação">
                <Row label="Título">{raq.titulo ?? '—'}</Row>
                <Row label="Unidade">
                  {raq.filial.codigo} · {raq.filial.nome}
                </Row>
                <Row label="Data da ocorrência">{fmtData(raq.dataIdentificacao)}</Row>
                <Row label="Emitente">{raq.criadoPor.nome}</Row>
                <Row label="Reincidente">
                  {raq.reincidente == null
                    ? '—'
                    : raq.reincidente
                      ? `Sim${raq.reincidenteVezes != null ? ` · ${raq.reincidenteVezes} vez(es)` : ''}`
                      : 'Não'}
                </Row>
                {raq.raqRelacionados.length > 0 && (
                  <Row label="RAQs relacionados">
                    <span className="flex flex-wrap gap-1.5">
                      {raq.raqRelacionados.map((r) => (
                        <span
                          key={r.id}
              ...
              <Section title="Fornecedor & material">
                <Row label="Fornecedor">
                  {raq.fornecedor.codigo} · {raq.fornecedor.razaoSocial}
                </Row>
                <Row label="Item (produto)">
                  {raq.produto
                    ? `${raq.produto.codigo} · ${raq.produto.descricao}`
                    : '—'}
                </Row>
                <Row label="Lote">
                  {raq.lotes[0]
                    ? `${raq.lotes[0].numero}${raq.lotes[0].quantidade != null ? ` (${String(raq.lotes[0].quantidade).replace('.', ',')})` : ''}`
                    : '—'}
                </Row>
                <Row label="Qtd. com defeito">
                  {raq.quantidadeDefeito != null
                    ? String(raq.quantidadeDefeito).replace('.', ',')
                    : '—'}
                </Row>
                <Row label="Tempo de parada">
                  {raq.tempoParadaMinutos != null
                    ? `${raq.tempoParadaMinutos} min`
                    : '—'}
                </Row>
                <Row label="Nota fiscal">
                  {raq.notasFiscais[0]?.numero ?? '—'}
                </Row>
                <Row label="Fabricação / validade">
                  {[
                    fmtData(raq.notasFiscais[0]?.dataFabricacao),
                    fmtData(raq.notasFiscais[0]?.dataValidade),
                  ]
                    .filter(Boolean)
                    .join(' · ') || '—'}
                </Row>
                <Row label="Recebimento">
                  {fmtData(raq.notasFiscais[0]?.dataRecebimento) || '—'}
                </Row>
                {(raq.transportador || raq.nomeMotorista) && (
                  <Row label="Transporte">
                    {[
                      raq.transportador,
                      [raq.placaCavalo, raq.placaCarreta].filter(Boolean).join(' / '),
                      raq.nomeMotorista,
                    ]
                      .filter(Boolean)
                      .join(' · ')}
              ...
              <Section title="Não conformidade">
                <Row label="Disposição">
                  {raq.disposicaoMaterial
                    ? `${raq.disposicaoMaterial.codigo} · ${raq.disposicaoMaterial.descricao}`
                    : '—'}
                </Row>
                <Row label="Origem">
                  {raq.origem ? `${raq.origem.codigo} · ${raq.origem.nome}` : '—'}
                </Row>
