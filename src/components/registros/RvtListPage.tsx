...
import { RvtWizard } from './RvtWizard'
import { RvtDetailPanel } from './RvtDetailPanel'
...
/** Lista dos Relatórios de Visita Técnica. */
export function RvtListPage() {
  const [items, setItems] = React.useState<Rvt[]>([])
  ...
  const [editing, setEditing] = React.useState<Rvt | null>(null)
  const [viewing, setViewing] = React.useState<Rvt | null>(null)
  ...
  const fetchPage = React.useCallback(
    async (opts: { status?: RncStatus | ''; page: number }) => {
      ...
      try {
        ...
        else setError('Não foi possível carregar os RVTs. A API está rodando?')
      } finally {
        setLoading(false)
      }
    },
  ...
  const handleDownload = async (r: Rvt) => {
  ...
  const atualizarItem = (atualizado: Rvt) => {
    ...
    <div className="flex flex-col gap-4 p-4 md:gap-6 md:p-6">
      <header className="flex flex-col gap-1">
        ...
        <p className="text-sm text-neutral-500">
          RVTs assinados pelos aprovadores configurados para o tipo e
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-neutral-200 bg-neutral-50/60 text-neutral-500">
                ...
                <th className="px-3 py-2.5 text-left font-medium">Pauta</th>
